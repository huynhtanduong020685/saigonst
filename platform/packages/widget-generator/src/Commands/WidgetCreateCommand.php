...
class WidgetCreateCommand extends Command
{
    ...
    /**
     * Search and replace all occurrences of ‘Module’ in all files with the name of the new module.
     */
    public function searchAndReplaceInFiles()
    {
        ...
        foreach ($manager->listContents('directory://', true) as $file) {
            if ($file['type'] === 'file') {
                $content = str_replace(['{widget}', '{Widget}',],
                    [strtolower($this->getWidget()), Str::studly($this->getWidget())],
