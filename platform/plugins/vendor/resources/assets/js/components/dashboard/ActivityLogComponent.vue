        ...
        <div class="tab-content">
            <div :class="[activeTab === activityLogTab ? 'tab-pane fade show active' : 'tab-pane fade']"
                 :id="activityLogTab">
                    ...
                    <div v-if="!isLoading && activityLogs.total === 0" class="tc light-gray-text pa3">
                        <div class="f5 mv2 b">{{ __('oops') }}</div>
                        <div class="f6 mv2">{{ __('no_activity_logs') }}</div>
                    </div>

                    <div v-if="!isLoading && activityLogs.total !== 0" class="pv3 ph3-ns">
                        <div v-for="activityLog in activityLogs.data" :key="activityLog.id" class="mb3">
                            <div class="dib light-gray-text">
                                ...
                                <span>(<a :href="'https://whatismyipaddress.com/ip/' + activityLog.ip_address" target="_blank" :title="activityLog.ip_address">{{ activityLog.ip_address }}</a>)</span>
                            </div>
                        </div>
                    </div>

                    <div v-if="!isLoading && activityLogs.next_page_url" class="tc light-gray-text">
                        <a class="f6 mv2" href="javascript:void(0)" v-if="!isLoadingMore" @click="getActivityLogs(activityLogs.next_page_url)">{{ __('load_more') }}</a>
    ...
    export default {
        ...
        methods: {
            ...
            getActivityLogs(url = null) {
                ...
                axios.get(url ? url : '/ajax/vendors/activity-logs')
                    .then((res) => {
                        let oldData = [];
